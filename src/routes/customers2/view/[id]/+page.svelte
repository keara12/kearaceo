<script>
    ...
    // Function ថ្មីសម្រាប់ Approve
    async function approveSchedule() {
        if (!confirm("តើអ្នកពិតជាចង់អនុម័តតារាងបង់ប្រាក់នេះមែនទេ?")) return;

        const formData = new FormData();
        formData.append('schedule', JSON.stringify(schedule));

        try {
            const response = await fetch('?/approveSchedule', {
                method: 'POST',
                body: formData
            });

            if (response.ok) {
                alert("ការអនុម័តទទួលបានជោគជ័យ!");
                await invalidateAll();
            } else {
                throw new Error();
            }
        } catch (err) {
            alert("មិនអាចអនុម័តបានទេ! សូមព្យាយាមម្តងទៀត។");
        }
    }
...
<div class="main-wrapper">
    <div class="header-actions no-print">
        ...
        <button onclick={approveSchedule} class="btn-approve">
            <span class="icon">✓</span> អនុម័ត (Approve)
        </button>
    ...
    <div class="schedule-paper">
        <div class="bank-header">
            ...
            <div class="report-title">
                ...
                <small style="color: #64748b;">(បង់ ១៥ ថ្ងៃម្ដង)</small>
            </div>
        </div>
        ...
        <div class="info-section">
            ...
            <div class="info-column">
                ...
                <div class="info-row"><span>រយៈពេលសរុប / Term:</span> <strong>{data.customer.loan_term} ខែ ({schedule.length} ដង)</strong></div>
            ...
            <tbody>
                {#each schedule as row, i}
                    <tr>
                        ...
                        <td class="text-right">{(row.beginning_balance || 0).toLocaleString()}៛</td>
                        <td class="text-right">{(row.pay_principal || 0).toLocaleString()}៛</td>
                        <td class="text-right">{(row.pay_interest || 0).toLocaleString()}៛</td>
                        <td class="text-right bold-text">{(row.total_pay || 0).toLocaleString()}៛</td>
                        <td class="text-right">{(row.ending_balance || 0).toLocaleString()}៛</td>
                        <td class="no-print">
                            <button 
...
<style>
    ...
    .header-actions { max-width: 800px; margin: 0 auto 20px auto; display: flex; justify-content: flex-end; gap: 10px; }
    .btn-back, .btn-print, .btn-approve { padding: 10px 20px; border-radius: 6px; border: none; cursor: pointer; font-weight: bold; display: flex; align-items: center; gap: 8px; font-family: 'Khmer OS Battambang'; }
    .btn-back { background: #64748b; color: white; }
    .btn-print { background: #1a3a8a; color: white; }
    .btn-approve { background: #10b981; color: white; } /* ពណ៌បៃតងសម្រាប់ Approve */

    .schedule-paper { background: white; width: 210mm; min-height: 297mm; margin: auto; padding: 40px; box-shadow: 0 0 20px rgba(0,0,0,0.2); box-sizing: border-box; }
    ...
    .bank-logo { background: #1a3a8a; color: white; width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; font-weight: bold; border-radius: 8px; font-size: 1.5rem; }
    ...
    .status-btn { border: none; padding: 3px 8px; border-radius: 4px; color: white; cursor: pointer; font-size: 0.7rem; width: 100%; font-family: 'Khmer OS Battambang'; }
    ...
    @media print {
        ...
        .schedule-paper { margin: 0 !important; padding: 0.5cm !important; box-shadow: none !important; width: 100% !important; }
