<script>
    ...
    async function approveLoan() {
        if (!confirm("តើអ្នកពិតជាចង់អនុម័ត និងរក្សាទុកតារាងនេះមែនទេ?")) return;

        const formData = new FormData();
        // បញ្ជូន schedule ដែលបានគណនារួចទៅ Server
        formData.append('full_schedule', JSON.stringify(schedule));
        ...
        try {
            ...
            if (response.ok) {
                alert("អនុម័ត និងរក្សាទុកទិន្នន័យរួចរាល់!");
                await invalidateAll();
            } else {
                throw new Error();
            }
...
<div class="main-wrapper">
    ...
    <button onclick={approveLoan} class="btn-approve">
        <span class="icon">✓</span> អនុម័តកម្ចី
    </button>
...
<style>
    ...
    .btn-back, .btn-print, .btn-approve { 
        padding: 10px 25px; 
        border-radius: 6px; 
        border: none; 
        cursor: pointer; 
    ...
    .btn-back:hover, .btn-print:hover, .btn-approve:hover {
        opacity: 0.9;
    }
