<script>
   ...
   function isPaidUpToDate(customer) {
        ...
        if (!schedule || schedule.length === 0) return true; // បើគ្មានតារាងបង់ប្រាក់ ចាត់ទុកថាធម្មតា
        ...
        for (let row of schedule) {
            ...
            // Logic ថ្មី៖ ពិនិត្យមើលតែខែចាស់ៗ និង ខែបច្ចុប្បន្ន
            // ប្រសិនបើកាលបរិច្ឆេទក្នុងតារាង ស្ថិតក្នុងខែនេះ ឬមុនខែនេះ
            if (
                rowDate.getFullYear() < currentYear || 
                (rowDate.getFullYear() === currentYear && rowDate.getMonth() <= currentMonth)
            ) {
                // បើមិនទាន់បង់ (Status មិនមែន "Paid") នោះនឹងបង្ហាញពណ៌ក្រហម (overdue)
                        ...
                        <tr>
                            ...
                            <td class="sticky-action">
                                <div class="action-btns">
                                    <a href="/customers1/view/{c.customer_id}" class="btn-view" title="មើល">👁️</a>
                                    <a href="/edit1/{c.customer_id}" class="btn-edit" title="កែប្រែ">✏️</a>
